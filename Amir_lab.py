...
def Logic_maker(old_percept, new_percept, action):
    """
    old_percept, new_percept: tuples like (1,0,1,0)
    action: one of 'W','A','S','D'
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# Main Pygame loop (no changes except using old_pos as a percept-tuple)
# ─────────────────────────────────────────────────────────────────────────────
cell_size = 80
pg.init()
...
while running and timestep < max_steps:
    ...
    # Draw agent (at its current coordinate)
    pg.draw.rect(
        disp,
        (0, 255, 0),
        (agent_pos[0] * cell_size + 5, agent_pos[1] * cell_size + 5, 70, 70)
    )
    pg.display.update()
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN:
            ...
            if moved and action is not None:
                # new_pos is actually a percept-tuple, not a coordinate
...
print("True final coordinate:", tuple(agent_pos))
print("Knowledge base (percept_history):")
for expr in percept_history:
    print("  ", expr)
print("Belief state (possible final coordinates):", final_pos_guesser(maze, percept_history))
