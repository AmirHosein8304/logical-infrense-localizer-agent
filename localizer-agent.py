...
symbol_names = [[f"wall_n{i}",f"wall_s{i}",f"wall_w{i}",f"wall_e{i}",f"in_s{i}"] for i in range(len(maze)*len(maze[0]))]
...
for i in range(len(maze)):
    for j in range(len(maze[0])):
        if maze[i][j] == 1:
            ...
            d_sym[(i*len(maze[0])+j)*5+4].subs({d_sym[(i*len(maze[0])+j)*5+4]:False})
        else:
            pg.draw.rect(disp, (0,0,255), (j*80, i*80, 80, 80),2)
            for k in range(4):
                ...
                if i+dr >= 0 and i+dr < len(maze) and j+dc >= 0 and j+dc < len(maze[0]):
                    if maze[i+dr][j+dc] == 1:
                        d_sym[(i*len(maze[0])+j)*5+k].subs({d_sym[(i*len(maze[0])+j)*5+k]:True})

same_sens = {sens(maze, 0, 0):[[i,j]]}
for i in range(len(maze)):
    for j in range(len(maze[0])):
        if maze[i][j] == 0:
            if sens(maze, i, j) == list(same_sens.keys())[-1]:
                same_sens[sens(maze, i, j)].append([i,j])
            else:
                same_sens[sens(maze, i, j)] = [[i,j]]
